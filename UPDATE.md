...
---
[2025-06-10] - 功能增强与体验优化：新增TFO选项，强化Clash转换，改进API与UI (Version 1.1.3) Written by Astolfo～(∠・ω< )

本次更新为 Proxy Provider Converter 带来了多项实用功能和体验提升。核心亮点包括为 Surge 和 Clash 转换新增了全局 TCP Fast Open (TFO) 选项，显著增强了针对 Clash 的转换能力（支持移除订阅信息节点、自定义节点名称后缀、强制TFO），并对API接口的健壮性和前端用户界面进行了优化。

> [!NOTE]
> Clash 用户请注意，本次更新修复了转换后配置文件可能因格式问题无法被部分 Clash核心正确解析的情况。

> [!TIP]
> 启用“强制添加 TFO 参数”可能有助于改善部分网络环境下的连接速度和延迟，但其效果因代理协议和服务器支持情况而异。

### ✨ 新增功能 (Features)

*   **全局 TCP Fast Open (TFO) 选项 (UX/DX 提升)**:
    *   前端新增“强制添加 tfo 参数”复选框。
    *   后端 API (`/api/convert`) 新增 `tfo` 查询参数。
    *   当启用时：
        *   **Surge**: 若节点本身未配置 TFO，则会尝试为转换后的 Surge 代理规则添加 `tfo=true`。
        *   **Clash**: 若节点本身未配置 TFO (如 `tfo: true` 或 `fast-open: true`)，则会为转换后的 Clash 代理节点添加 `tfo: true`。
*   **Clash 转换功能增强 (UX/DX 提升)**:
    *   **移除节点名称中的订阅信息**: 新增逻辑以识别并移除 Clash 代理节点名称中包含的特定订阅信息关键词（如“过期”、“流量”、“官网”等），通过前端“在列表中删除订阅信息”复选框控制。
    *   **为节点名称添加自定义后缀**: 当启用“启用订阅名称”并填写名称后，该名称会作为后缀附加到每个 Clash 代理节点的名称之后。
    *   **强制 TFO 支持**: 如上所述，Clash 转换现在也支持强制添加 TFO 参数。

### 🛠️ 优化与修复 (Improvements & Fixes)

*   **功能修复 - Clash 输出格式修复 (Bug修复/体验优化)**:
    *   当转换目标为 Clash 时，输出的配置文件内容中，注释行 `# Subscription URL: ...` 之后会增加一个换行符。此修改旨在修复之前可能导致部分 Clash 核心无法正确解析转换后配置文件的问题，确保了更好的兼容性。
*   **API 参数校验增强 (健壮性提升)**:
    *   `/api/convert` 接口现在会对 `target` 参数进行校验，仅接受 "clash" 或 "surge"，对无效值会返回 400 错误，提升了API的稳定性。
*   **前端用户界面优化 (UX 提升)**:
    *   “在列表中加入订阅信息” (`displaySubInfo`) 复选框现在仅在转换目标为 "Surge" 时可见，因其主要影响 Surge 的输出。
    *   “在列表中删除订阅信息” (`removeSubInfo`) 复选框调整为通用选项，对 Clash 和 Surge 均生效。
    *   调整了前端选项的布局顺序，使其更符合逻辑。
*   **订阅信息关键词匹配优化 (准确性提升)**:
    *   用于识别节点名称中订阅信息的关键词列表 (`expiryNameList`) 中增加了 `"Expire"`，以更全面地匹配可能的表述。
